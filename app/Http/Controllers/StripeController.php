...
class StripeController extends Controller
{
    ...
    public function handleStripeCallback(Request $request)
    {
        ...
        // Get Stripe secret key from system settings (same source as StripeCheckoutService)
        $stripeSecretKey = HelperService::systemSettings('stripe_secret_key');
        if (empty($stripeSecretKey)) {
            Log::error('Stripe callback: stripe_secret_key is not configured in system settings');
            if ($type === 'app') {
                return ApiResponseService::errorResponse('Stripe is not configured', ['status' => 'error'], 500);
            }
